...
export default function ManageQuizMasters() {
  ...
  const fetchQuizMasters = async () => {
    try {
      ...
      const res = await axios.get(
        "http://localhost:3000/api/quizMaster/get-quizmaster",
        {
  ...
  const deleteQuizMaster = async (id) => {
    ...
    try {
      await axios.delete(
        `http://localhost:3000/api/quizMaster/delete-quizmaster/${id}`,
        {
          withCredentials: true,
        }
      );
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      >
        <h2 className="text-lg justify-center font-semibold mb-4">Add New Quiz Master</h2>
