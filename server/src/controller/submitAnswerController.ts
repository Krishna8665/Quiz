...
export const submitAnswer = async (req: SubmitAnswerRequest, res: Response) => {
  try {
    const { quizId, roundNumber, teamId, questionId, answerId, isPassed } = req.body;
    ...
    if (!quizHistory) {
      const quiz = await Quiz.findById(quizId).populate("rounds").populate("teams");
    ...
    const roundPoints = roundAnswers.reduce((sum, a) => sum + a.pointsEarned, 0);

    const roundStats = quizHistory.rounds.find(r => r.roundNumber === roundNumber);
    const teamStats = roundStats?.teams.find(t => String(t.teamId) === String(teamId));
    if (teamStats) {
    ...
    const totalStats = quizHistory.total.find(t => String(t.teamId) === String(teamId));
    if (totalStats) {
