...
export const getQuizHistory = async (req: Request, res: Response) => {
  try {
    ...
    console.log(`Found ${records.length} history records for quizId: ${quizId}`);
    
    // Debug: Show what rounds exist
    const uniqueRounds = [...new Set(records.map((r) => r.roundId?.roundNumber))];
    ...
    if (teamId) {
      ...
      const roundWiseStats = records.map((record) => {
        ...
        return {
          ...
          roundName: record.roundId.name || `Round ${record.roundId.roundNumber}`,
