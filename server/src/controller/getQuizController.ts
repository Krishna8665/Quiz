...
export const getQuizHistory = async (req: Request, res: Response) => {
  try {
    ...
    sessions.forEach((s) => {
      sessionInfo[s._id.toString()] = {
        startedAt: s.createdAt,
        endedAt: s.status === "completed" ? s.updatedAt : undefined,
      };
    });
    ...
    const histories = (await QuizHistory.find({ quizId })
      .populate("teamId")
      .populate("roundId")
      .populate("startedBy")) as PopulatedQuizHistory[];
    ...
    histories.forEach((h) => {
      ...
      const existingTeam = sessionMap[sessionId].teams.find(
        (t) => t.teamId === teamId
      );
